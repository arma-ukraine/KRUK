...

{
	...
	if (_light != "") then {
		_container addItemCargoGlobal [_light, 1]
	};
	if (_optic != "") then {
		_container addItemCargoGlobal [_optic, 1]
	};
	if (count _magazineArray > 1) then {
		_container addMagazine _magazineArray
	};
	...
	if (count _x > 5) then {
		...
		if (_something isEqualType []) then {
			if (count _something > 1) then {
				_container addMagazine _something
			};
		} else {
			if (_something != "") then {
				_container addItemCargoGlobal [_something, 1]
			};
		};
	};
