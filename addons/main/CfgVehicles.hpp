class CfgVehicles
{
    ...
    class GVAR(moduleBuyer) : Module_F
    {
        displayName = QGVAR(moduleBuyer);
        function = QFUNC(moduleBuyer);
        init = QFUNC(moduleBuyer);
        category = QGVAR(modules);
        scope = 2;        // EDEN: 1 - hide, 2 - show
        scopeCurator = 1; // ZEUS: 1 - hide, 2 - show
        ...
        {
            // Module-specific arguments:
            class BuyPriceMultiplier : Edit
            {
                property = QGVAR(moduleBuyPriceMultiplier);
